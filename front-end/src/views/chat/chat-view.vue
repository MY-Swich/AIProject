...
import SessionItem from '@/views/chat/components/session-item.vue'
import { type AiMessage, useChatStore } from '@/views/chat/store/chat-store'
...
const { handleDeleteSession, handleCreateSession, handleUpdateSession, handleClearMessage } =
  chatStore
...
onMounted(async () => {
  ...
  api.aiSessionController.findByUser().then((res) => {
    ...
    if (sessionList.value.length > 0) {
      ...
      handleCreateSession({ name: '新的聊条（一）' })
    }
    loading.value = false
  })
})
      ...
      <div class="session-panel">
        ...
        <div class="button-wrapper">
          <el-button
            ...
            :icon="ChatRound"
            size="small"
            @click="handleCreateSession({ name: '新的聊天' })"
            >创建会话
          </el-button>
        </div>
      </div>
